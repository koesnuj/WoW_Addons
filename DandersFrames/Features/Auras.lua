...
-- PERF FIX: Per-unit debounce for triple-hook scenario.
-- CompactUnitFrame_UpdateAuras may internally call UpdateBuffs + UpdateDebuffs,
-- causing CaptureAurasFromBlizzardFrame to fire 3x per aura change per unit.
-- This table tracks which units were already processed this game tick.
local captureProcessedThisTick = {}
local captureClearPending = false
...
local function CaptureAurasFromBlizzardFrame(frame, triggerUpdate)
    ...
    -- PERF FIX (string cache): Cache frame relevance so we never repeat
    -- nameplate/preview/settings string checks on the same frame object.
    -- The flag is set once per frame lifetime (frame identity doesn't change).
    local dominated = frame._dfAuraRelevant
    if dominated == false then return end
    if dominated == nil then
        -- First encounter with this frame — run the full string checks once
        local unit = frame.unit
        if unit and type(unit) == "string" and unit:find("nameplate") then
            frame._dfAuraRelevant = false
            return
        end
        ...
        if frameName and (frameName:find("Preview") or frameName:find("Settings") or frameName:find("NamePlate")) then
            frame._dfAuraRelevant = false
            return
        end
        frame._dfAuraRelevant = true
    end
    ...
    -- PERF FIX (debounce): Skip if this unit was already captured this game tick.
    -- Prevents 3x redundant cache rebuilds from the triple-hook on
    -- UpdateAuras / UpdateBuffs / UpdateDebuffs.
    if captureProcessedThisTick[unit] then return end
    captureProcessedThisTick[unit] = true
    if not captureClearPending then
        captureClearPending = true
        C_Timer.After(0, function()
            wipe(captureProcessedThisTick)
