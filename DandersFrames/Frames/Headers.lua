...
-- PERF FIX: Cached pinned-frame lookup.
-- FindPinnedFrameForUnit is called on every high-frequency unit event
-- (UNIT_HEALTH, UNIT_AURA, UNIT_POWER_UPDATE, etc.).  The old implementation
-- iterated header:GetAttribute("child"..i) for 2 headers × 40 children = 80
-- C calls PER event.  In a 20-person raid during combat this adds up to
-- thousands of GetAttribute calls per second.
--
-- New approach: build a unit→pinnedFrame map lazily on first access each game
-- tick, then serve O(1) lookups for the rest of the tick.  The map is
-- invalidated at the start of the next tick via C_Timer.After(0).
local pinnedUnitMap = nil  -- nil = stale, table = valid for current tick
local pinnedMapClearPending = false

local function FindPinnedFrameForUnit(unit)
    if not DF.PinnedFrames or not DF.PinnedFrames.initialized or not DF.PinnedFrames.headers then
        return nil
    end
    if not pinnedUnitMap then
        pinnedUnitMap = {}
        for setIndex = 1, 2 do
            local header = DF.PinnedFrames.headers[setIndex]
            if header and header:IsShown() then
                for i = 1, 40 do
                    local child = header:GetAttribute("child" .. i)
                    if child and child:IsVisible() and child.unit then
                        pinnedUnitMap[child.unit] = child
                    end
                end
            end
        end
        -- Schedule invalidation so the map is rebuilt fresh next tick
        if not pinnedMapClearPending then
            pinnedMapClearPending = true
            C_Timer.After(0, function()
                pinnedUnitMap = nil
                pinnedMapClearPending = false
            end)
        end
    end
    return pinnedUnitMap[unit]
end
